<?php
    if (count($_POST)!=0){
        ...
        if ($cadeaErros == ""){
            setcookie("marcaModelo",$_POST["marMod"]);
            setcookie("numKm",$_POST["nKm"]);
            setcookie("urlFabricante",$_POST["urlFab"]);
            setcookie("combustible",$_POST["com"]);
            for ($i=0; $i<count($_POST["eqEx"]); $i++)                
                setcookie("equipEx[$i]",$_POST["eqEx"][$i]);                                 
        }
    }
?>
<?php 
